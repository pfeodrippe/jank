...
int main()
{
  ...
  std::vector<char const *> args = { "clang-repl", "-std=c++17" };
  ...
  if(!Interp)
  {
    std::cerr << "Failed to create interpreter: " << llvm::toString(Interp.takeError())
              << std::endl;
    return 1;
  }
  ...
  auto R2 = (*Interp)->ParseAndExecute("int add(int a, int b) { return a + b; }");
  if(R2)
  {
