...
namespace jank::perf
{
  ...
  TEST_SUITE("Eval Performance")
  {
     ...
     *
     * This test verifies that the persistent JIT cache infrastructure works:
     * 1. With jit_cache DISABLED but persistent_jit_cache ENABLED
     * 2. C++ source files are saved to disk with correct hashes
     * 3. Metadata files contain correct qualified and unique names
     *
     * Note: This tests the SAVE infrastructure. Loading from disk to skip JIT
     * requires object file caching, which is a future enhancement.
     * ========================================================================== */
    TEST_CASE("Persistent JIT Cache: Infrastructure Test")
    {
      std::cout << "\n=== Persistent JIT Cache: Infrastructure Test ===\n";
      std::cout << "(Tests saving C++ source to disk with jit_cache disabled)\n\n";

      /* Save original settings */
      auto const original_jit_cache = util::cli::opts.jit_cache_enabled;
      auto const original_persistent_cache = util::cli::opts.persistent_jit_cache_enabled;

      /* Disable in-memory cache, enable persistent cache */
      util::cli::opts.jit_cache_enabled = false;
      util::cli::opts.persistent_jit_cache_enabled = true;

      /* Clear the persistent cache to start fresh */
      __rt_ctx->persistent_jit_cache.clear();
      auto const initial_stats = __rt_ctx->persistent_jit_cache.get_stats();
      std::cout << "Initial cache entries: " << initial_stats.entries << "\n";

      constexpr int NUM_FUNCTIONS = 5;

      /* Generate unique namespace */
      auto const test_ns = "persistent-test-" + std::to_string(
        std::chrono::steady_clock::now().time_since_epoch().count() % 10000);

      /* Create namespace */
      __rt_ctx->eval_string(("(ns " + test_ns + ")").c_str());

      /* Step 1: Define functions - these should save to disk */
      std::cout << "\n-- Step 1: Define " << NUM_FUNCTIONS << " functions (JIT + save to disk) --\n";
      auto const define_start{ high_resolution_clock::now() };

      for(int i = 0; i < NUM_FUNCTIONS; i++)
      {
        std::string code = "(defn persistent-fn-" + std::to_string(i) + " [x] (+ x " + std::to_string(i * 10) + "))";
        __rt_ctx->eval_string(code.c_str());
      }

      auto const define_end{ high_resolution_clock::now() };
      auto const define_time_ms = duration_cast<milliseconds>(define_end - define_start).count();

      auto const after_define_stats = __rt_ctx->persistent_jit_cache.get_stats();
      std::cout << "  Time: " << define_time_ms << " ms\n";
      std::cout << "  Disk entries after define: " << after_define_stats.entries << "\n";

      /* Verify files were created */
      CHECK_MESSAGE(after_define_stats.entries >= NUM_FUNCTIONS,
                    "Expected at least " << NUM_FUNCTIONS << " cache entries, got " << after_define_stats.entries);

      /* Step 2: Test that the functions work correctly */
      std::cout << "\n-- Step 2: Verify functions work correctly --\n";
      for(int i = 0; i < NUM_FUNCTIONS; i++)
      {
        std::string call_code = "(" + test_ns + "/persistent-fn-" + std::to_string(i) + " 5)";
        auto result = __rt_ctx->eval_string(call_code.c_str());

        /* Result should be 5 + (i * 10) */
        auto expected = 5 + (i * 10);
        auto actual = expect_object<obj::integer>(result)->data;
        CHECK_MESSAGE(actual == expected,
                      "Function " << i << " returned " << actual << ", expected " << expected);
      }
      std::cout << "  All functions work correctly!\n";

      /* Step 3: Redefine functions with jit_cache still disabled */
      std::cout << "\n-- Step 3: Redefine functions (no in-memory cache, full JIT) --\n";
      auto const redefine_start{ high_resolution_clock::now() };
      ...
      auto const redefine_end{ high_resolution_clock::now() };
      auto const redefine_time_ms = duration_cast<milliseconds>(redefine_end - redefine_start).count();
      std::cout << "  Time: " << redefine_time_ms << " ms (full JIT each time)\n";

      /* Summary */
      std::cout << "\n=== Results ===\n";
      std::cout << "| Operation               | Time (ms) |\n";
      std::cout << "|-------------------------|-----------|\n";
      std::cout << "| Initial define          | " << std::setw(9) << define_time_ms << " |\n";
      std::cout << "| Redefine (no cache)     | " << std::setw(9) << redefine_time_ms << " |\n";
      std::cout << "|-------------------------|-----------|\n";
      std::cout << "| Disk cache entries      | " << std::setw(9) << after_define_stats.entries << " |\n";

      std::cout << "\n✓ Persistent cache infrastructure is working!\n";
      std::cout << "  C++ source and expressions are being saved to disk.\n";
      std::cout << "  Future: compile_to_object() will enable loading from .o files.\n";

      /* Restore original settings */
      util::cli::opts.jit_cache_enabled = original_jit_cache;
      util::cli::opts.persistent_jit_cache_enabled = original_persistent_cache;
    }

    /* ==========================================================================
     * Persistent JIT Cache: Cross-Session Performance Test
     *
     * This test demonstrates the persistent JIT cache with object file loading:
     * - First eval (cold): Full compilation + save to disk + compile to .o
     * - Cross-session: Load .o file + call factory (skips JIT entirely)
     *
     * The massive speedup comes from skipping JIT entirely via pre-compiled .o files.
     * ========================================================================== */
    TEST_CASE("Persistent JIT Cache: Performance Comparison")
    {
      std::cout << "\n=== Persistent JIT Cache: Cross-Session Performance ===\n";
      std::cout << "(Measures first eval vs cross-session .o file load)\n\n";
      ...
      auto const test_ns = "pcache-perf-" + std::to_string(
        std::chrono::steady_clock::now().time_since_epoch().count() % 10000);
      __rt_ctx->eval_string(("(ns " + test_ns + ")").c_str());

      /* A bigger, more complex function to measure */
      std::string big_defn = R"(
        (defn complex-calc [x y z]
          (let [a (* x x)
                b (* y y)
                c (* z z)
                sum (+ a b c)
      ...
      /* ==========================================================================
       * First Eval (Cold): Full compilation + save to cache
       * This is what happens on first jank session
       * ========================================================================== */
      std::cout << "-- First Eval (Cold): Full pipeline --\n";
      std::cout << "  (parse + analyze + codegen + JIT + save + compile to .o)\n";

      auto const first_start{ high_resolution_clock::now() };
      __rt_ctx->eval_string(big_defn.c_str());
      auto const first_end{ high_resolution_clock::now() };
      auto const first_time_us = duration_cast<microseconds>(first_end - first_start).count();

      auto stats = __rt_ctx->persistent_jit_cache.get_stats();
      std::cout << "  Time: " << first_time_us << " us (" << first_time_us / 1000 << " ms)\n";
      std::cout << "  Cache entries: " << stats.entries << "\n";

      /* Verify function works */
      auto result = __rt_ctx->eval_string(("(" + test_ns + "/complex-calc 3 4 5)").c_str());
      std::cout << "  Function result: " << runtime::to_string(result) << "\n";

      /* ==========================================================================
       * Compile cached entries to object files for test
       * (This simulates running `jank --compile-cache` offline)
       * ========================================================================== */
      auto const cache_dir = __rt_ctx->persistent_jit_cache.cache_dir();
      std::vector<u64> hashes;

      /* First pass: collect hashes from .cpp files */
      for(auto const &file : std::filesystem::directory_iterator{ cache_dir })
      {
        if(file.path().extension() == ".cpp")
        {
          auto const hash_str = file.path().stem().string();
          hashes.push_back(std::stoull(hash_str, nullptr, 16));
        }
      }

      /* Compile each cached entry to object file */
      std::cout << "\n-- Compiling cache entries to object files --\n";
      for(auto hash : hashes)
      {
        auto const compiled = __rt_ctx->persistent_jit_cache.compile_to_object(hash);
        std::cout << "  Hash " << std::hex << hash << std::dec
                  << ": " << (compiled ? "OK" : "skipped") << "\n";
      }

      /* Second pass: count object files */
      size_t object_files_count = 0;
      ...
      for(auto const &file : std::filesystem::directory_iterator{ cache_dir })
      {
        ...
        {
          object_files_count++;
          total_obj_size += std::filesystem::file_size(file.path());
        }
      }

      std::cout << "\n-- Cache Contents --\n";
      std::cout << "  C++ source files: " << hashes.size() << "\n";
      std::cout << "  Object files (.o): " << object_files_count << "\n";
      if(total_obj_size > 0)
      {
        std::cout << "  Total object size: " << (total_obj_size / 1024) << " KB\n";
      }

      /* ==========================================================================
       * Cross-Session Load via Jank Eval
       * Simulate a new session by clearing in-memory cache and re-evaluating.
       * The persistent cache should load from .o files automatically.
       * ========================================================================== */
      std::cout << "\n-- Cross-Session Load (via jank eval) --\n";
      std::cout << "  Simulating new session: clear in-memory cache, re-evaluate same code\n";

      /* Clear the in-memory JIT cache to force re-evaluation path */
      __rt_ctx->jit_cache.clear();

      /* Re-evaluate the same function definition.
       * This time it should load from .o file instead of JIT compiling. */
      auto const cross_start{ high_resolution_clock::now() };
      __rt_ctx->eval_string(big_defn.c_str());
      auto const cross_end{ high_resolution_clock::now() };
      auto const cross_session_time_us = duration_cast<microseconds>(cross_end - cross_start).count();

      std::cout << "  Re-eval time: " << cross_session_time_us << " us ("
                << cross_session_time_us / 1000 << " ms)\n";

      /* Verify function still works */
      auto result2 = __rt_ctx->eval_string(("(" + test_ns + "/complex-calc 3 4 5)").c_str());
      std::cout << "  Function result (after reload): " << runtime::to_string(result2) << "\n";

      /* Check cache stats */
      auto const stats2 = __rt_ctx->persistent_jit_cache.get_stats();
      std::cout << "  Disk hits: " << stats2.disk_hits << ", misses: " << stats2.disk_misses << "\n";

      /* ==========================================================================
       * Results Summary
       * ========================================================================== */
      double speedup = first_time_us > 0 && cross_session_time_us > 0
                         ? static_cast<double>(first_time_us) / static_cast<double>(cross_session_time_us)
                         : 1.0;

      /* Time saved */
      auto const time_saved_us = first_time_us - cross_session_time_us;
      auto const time_saved_pct = first_time_us > 0
                                    ? (100.0 * time_saved_us / first_time_us)
                                    : 0.0;

      std::cout << "\n=== Results ===\n";
      std::cout << "| Scenario                | Time (us)   | Time (ms) |\n";
      std::cout << "|-------------------------|-------------|-----------|\n";
      std::cout << "| First eval (full JIT)   | " << std::setw(11) << first_time_us
                << " | " << std::setw(9) << first_time_us / 1000 << " |\n";
      std::cout << "| Cross-session (.o load) | " << std::setw(11) << cross_session_time_us
                << " | " << std::setw(9) << cross_session_time_us / 1000 << " |\n";
      std::cout << "|-------------------------|-------------|-----------|\n";
      std::cout << "| Time saved              | " << std::setw(11) << time_saved_us
                << " | " << std::setw(8) << std::fixed << std::setprecision(1)
                << time_saved_pct << "% |\n";
      std::cout << "| Speedup                 | " << std::setw(8) << std::fixed
                << std::setprecision(0) << speedup << "x |           |\n";

      if(object_files_count > 0)
      {
        std::cout << "\n✓ Object file cache is working!\n";
        std::cout << "  - First eval: " << first_time_us / 1000 << " ms (full JIT pipeline)\n";
        std::cout << "  - Cross-session: " << cross_session_time_us << " us (load .o + call factory)\n";
        std::cout << "  - Speedup: " << static_cast<int>(speedup) << "x\n";
      }
      else
      {
        std::cout << "\n⚠ No object files found in cache.\n";
        std::cout << "  Check compile_to_object() implementation.\n";
      }
