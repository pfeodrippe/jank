...
namespace jank::nrepl_server::asio
{
  TEST_SUITE("nREPL native header macros")
  {
    TEST_CASE("enumerate_native_header_macros returns macros from C header")
    {
      ...
      bool found_test_white
        = std::find(all_macros.begin(), all_macros.end(), "TEST_WHITE") != all_macros.end();
      bool found_test_pi
        = std::find(all_macros.begin(), all_macros.end(), "TEST_PI") != all_macros.end();
      bool found_key_escape
        = std::find(all_macros.begin(), all_macros.end(), "KEY_ESCAPE") != all_macros.end();
    ...
    TEST_CASE("enumerate_native_header_macros filters by prefix")
    {
      ...
      bool found_key_escape
        = std::find(test_macros.begin(), test_macros.end(), "KEY_ESCAPE") != test_macros.end();
   ...
   * e.g., (rl/KEY_ESCAPE) just returns the macro value. */
    ...
    TEST_CASE(
      "get_native_header_macro_param_count returns parameter count for function-like macros")
    {
      ...
      /* Non-existent macro should return nullopt */
      auto nonexistent_count
        = get_native_header_macro_param_count(alias, "NONEXISTENT_MACRO_12345");
      CHECK(!nonexistent_count.has_value());
    }

    TEST_CASE(
      "get_native_header_macro_expansion includes parameter signature for function-like macros")
    {
   ...
   * which should evaluate to 5 + 10 = 15 */
      ...
      REQUIRE(value_it != responses.front().end());
      std::cerr << "(th/TEST_ADD (th/test_get_five) (th/test_get_ten)) = "
   ...
   * which should evaluate to 5 + 10 = 15 */
      ...
      REQUIRE(value_it != responses.front().end());
      std::cerr << "(let [a (th/test_get_five)] (th/TEST_ADD a 10)) = "
                << value_it->second.as_string() << "\n";
      CHECK(value_it->second.as_string().find("15") != std::string::npos);
   ...
   * test_get_int_ptr() returns pointer to static int (42), so *ptr + 8 = 50 */
    ...
    TEST_CASE("enumerate_native_header_macros includes function-like macros")
    {
      ...
      bool found_pi
        = std::find(test_macros.begin(), test_macros.end(), "TEST_PI") != test_macros.end();
      bool found_white
      ...
      bool found_add
        = std::find(test_macros.begin(), test_macros.end(), "TEST_ADD") != test_macros.end();
      bool found_mul
        = std::find(test_macros.begin(), test_macros.end(), "TEST_MUL") != test_macros.end();
      bool found_clamp
